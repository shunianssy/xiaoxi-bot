...
- 完成主程序配置与模型配置界面重构、模型提供商与小熙适配器配置重构（引入 TOML 校验）、WebSocket 认证逻辑抽取为共享模块统一 WS 端点，升级 React 到 19.2.1 并更新依赖，WebUI 配置与可视化全部迁移到主配置及模型配置中，优化配置更新提示、插件详情页面和路径安全校验，并增强模型与梦境等多项配置的可视化和自动检验。
...
## [0.11.5] - 2025-11-21
### 🌟 重大更新
- WebUI 现支持手动重启小熙，曲线救国版“热重载”
- 新增小熙 QQ 适配器可视化编辑 UI（独立进程，需手动上传/下载并覆盖适配器文件）
- 小熙主程序配置支持可视化模式与源代码模式双模式编辑，后端执行 TOML 校验
...
- 主动发言功能，小熙会自主提出问题（可精细调控频率）
- 支持多重人格设定，可以随机切换成不同状态
- 新增表达方式学习新模式，更少的占用
- 添加表情包管理插件
- 现可更好的支持多平台
- 添加deepthink插件（默认关闭），让小熙可以深度思考一些问题
...
- **回复优化**: 优化reply，填补缺失值，让小熙可以回复自己的消息
...
#### 🎤 语音消息支持
- **Voice消息处理**: 新增对voice类型消息的支持，小熙现在可以识别和处理语音消息（需要模型配置）

#### 全新情绪系统
- **持续情绪**: 小熙现在拥有持续的情绪状态，情绪会影响回复风格和行为
...
### 👥 人物印象系统
**小熙现在能认得群友，记住每个人的特点**
- **人物侧写功能**: 加入了人物侧写！小熙现在能认得群友，新增用户侧写功能，将印象拆分为多方面特点
...
- 现在，你可以自定义小熙的表达方式，并且小熙也可以学习群友的聊天风格（需要在配置文件中打开）
- 不再需要繁琐的安装MongoDB！弃用MongoDB，采用轻量sqlite,无需额外安装（提供数据迁移脚本）
- focus模式初步支持了插件，我们提供了两个示例插件（需要手动启用），可以让小熙实现更丰富的操作。禁言插件和豆包绘图插件是示例用插件。
...
- 为专注模式添加关系线索
- 在专注模式下，小熙可以决定自行发送语音消息（需要搭配tts适配器）
- 优化reply，减少复读
- 可自定义连续回复次数
- 可自定义处理器超时时间
...
- **智能交互模式**: `SubHeartflow` 现在可以根据情境智能选择不同的交互模式：
    ...
    - **离线模式 (Offline/Absent)**: 在特定情况下，小熙可能会选择暂时不查看或回复群聊消息。
...
- **重复性检查**: 加入心流回复重复性检查机制，防止小熙陷入固定回复模式。

#### 全新知识库系统 (New Knowledge Base System - LPMM)
- **引入 LPMM**: 新增了 **LPMM (Large Psychology Model Maker)** 知识库系统，具有强大的信息检索能力，能显著提升小熙获取和利用知识的效率。
- **功能集成**: 集成了 LPMM 知识库查询功能，进一步扩展信息检索能力。
- **推荐使用**: 强烈建议使用新的 LPMM 系统以获得最佳体验。旧的知识库系统仍然可用作为备选。

#### 昵称系统 (Nickname System)
- **自动取名**: 小熙现在会尝试给群友取昵称，减少对易变的群昵称的依赖，从而降低认错人的概率。
...
- **积极性配置**: 优化"让小熙更愿意说话"的相关配置，使机器人更积极参与对话。
...
- **运行状态查看**: 提供了一个非常基础的图形用户界面，用于查看小熙的运行状态。
...
- **独立适配器**: 提供了一个"桌宠"独立适配器，用于连接小熙和桌宠。
- **获取方式**: 可在 MaiBot 的 GitHub 组织中找到该适配器，不包含在主程序内。

#### 一键包内容 (One-Click Package Contents)
- **辅助程序**: 一键包中包含了简易 GUI 和 **小熙帮助配置** 等辅助程序，后者可在配置出现问题时提供帮助。
