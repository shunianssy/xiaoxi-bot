...
Action是给小熙在回复之外提供额外功能的智能组件，**由小熙的决策系统自主选择是否使用**，具有随机性和拟人化的调用特点。Action不是直接响应用户命令，而是让小熙根据聊天情境智能地选择合适的动作，使其行为更加自然和真实。

### Action的特点

- 🧠 **智能激活**：小熙根据多种条件智能判断是否使用
...
> 设计目的：在加载许多插件的时候降低LLM决策压力，避免让小熙在过多的选项中纠结。

**第一层：激活控制（Activation Control）**

激活决定小熙是否 **“知道”** 这个Action的存在，即这个Action是否进入决策候选池。不被激活的Action小熙永远不会选择。

**第二层：使用决策（Usage Decision）**

在Action被激活后，使用条件决定小熙什么时候会 **“选择”** 使用这个Action。
...
| 激活类型 | 说明 | 使用场景 |
| ----------- | ---------------------------------------- | ---------------------- |
| [`NEVER`](#never-激活)     | 从不激活，Action对小熙不可见               | 临时禁用某个Action      |
| [`ALWAYS`](#always-激活)    | 永远激活，Action总是在小熙的候选池中        | 核心功能，如回复、不回复 |
...
1. **第一层激活判断**：
    ...
    - 使用随机数进行决策，当`random.random() < self.random_activation_probability`时，小熙才"知道"可以使用这个Action
2. **第二层使用决策**：

   - 即使Action被激活，小熙还会根据 `action_require` 中的条件判断是否真正选择使用
   - 例如：如果刚刚已经发过表情，根据"不要连续发送多个表情"的要求，小熙可能不会选择这个Action
